...
if run == 5:
    ...
    if user >= 200:
        ...
        input("Number must be equal to or greater than 200. Please enter a valid number: ")
...
if run == 9:
    user = input("Enter a number to check if the number given is a prime number: ")
    user = int(user)
    if user / 1 == user:
        if user / user == 1:
            print(f"the number {user} is indeed is a prime number!")
        else:
